    ...
    <script>
        ...
        function waitForElement(selector, timeoutMs = 5000) {
            return new Promise((resolve, reject) => {
                ...
                (function check() {
                    ...
                    if (performance.now() - start > timeoutMs) return reject(new Error(
        ...
        class Card {
            ...
            render() {
                ...
                const hasDates = startDate instanceof Date && !isNaN(startDate) && endDate instanceof Date && !isNaN(
                    endDate);
                const hasMembers = Array.isArray(this.members) && this.members.length > 0;
            ...
            openArchiveModal() {
                ...
                if (modal) {
                    ...
                    if (message) message.textContent =
                        'Are you sure you want to archive this card? You can restore it later from the archive.';
                }
...
@can('assign-tasks')
    <template is-modal="assignTask">
        <div class="flex flex-col items-center justify-center w-full h-full gap-6 p-4">
            ...
            {{-- Assign Form --}}
            <form class="flex flex-col items-center justify-center w-full gap-6" method="POST"
                data-role="assign-task-form">
                @csrf
            ...
            <form class="flex flex-col items-center justify-center w-full gap-6" method="POST"
                ...
                <x-form.button type="submit" class="w-full bg-orange-500 hover:bg-orange-600">Unassign
                    Member</x-form.button>
            </form>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (typeof ModalView !== 'undefined') {
            ...
            ModalView.onShow('assignTask', (modal) => {
                ...
                const cardElement = document.querySelector('.is-dragging') || document.querySelector(
                    ...
                    form => {
                        form.addEventListener("submit", (e) => {
                            ...
                            if (!selectElement || !selectElement.value) {
                                ...
                                if (typeof ToastView !== 'undefined') {
                                    ToastView.notif('Warning',
                                        'Please select a member to unassign');
                                }
                                return false;
                            }
                            if (typeof PageLoader !== 'undefined') {
